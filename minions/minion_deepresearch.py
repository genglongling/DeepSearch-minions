...
from minions.utils import serp_util, firecrawl_util
...
class MinionDeepResearch:
    ...
    def __call__(
        ...
        print("[INFO] Retrieving links from SerpAPI for query...")
        search_results = serp_util.search_serp(task, num_results=max_urls)
        urls = [result['link'] for result in search_results]

        print("[INFO] Crawling pages with Firecrawl...")
        all_text = []
        for url in urls:
            print(f"[Crawling] {url}")
            try:
                page = firecrawl_util.scrape_url(url)
                page_text = page.get("markdown") or page.get("html") or ""
        ...
        #summary = self.local_client.summarize(combined_text)

        # Join context sections
        context = "\n\n".join(context)

        print("[INFO] Synthesizing answer with remote model...")
        context = context + combined_text  # append summary to existing context if any
